...
class YSLHeelWidget extends StatefulWidget {
  final String categoryName;

  YSLHeelWidget({required this.categoryName});

  @override
  _YSLHeelWidgetState createState() => _YSLHeelWidgetState();
}

class _YSLHeelWidgetState extends State<YSLHeelWidget> {
  late Future<List<Map<String, dynamic>>> yslHeels;

  @override
  void initState() {
    super.initState();
    yslHeels = fetchYSLHeelData();
  }

  Future<List<Map<String, dynamic>>> fetchYSLHeelData() async {
    final response = await Supabase.instance.client
        .from('products')  // Fetch from the products table
        .select('id, name, image_url, description, price, favorite, rating, category')
        .eq('category', 'ysl_heels');
    ...
    for (var item in data) {
      item['name'] = item['name'] ?? 'YSL Heel';
      item['image_url'] = item['image_url'] ?? 'https://via.placeholder.com/150';
      item['description'] = item['description'] ?? 'No description available';
      item['price'] = item['price'] ?? 0.0;
      item['favorite'] = item['favorite'] ?? false; // Ensure favourite is set
      item['rating'] = item['rating'] ?? 0.0; // Default rating
    }
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(child: Text('No YSL Heels available.'));
          } else {
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Available YSL Heels in ${widget.categoryName}',
                    style: TextStyle(
                      fontSize: 22,
